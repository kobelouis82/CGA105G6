...
<center>
	<h2>
		<a class="content"
			href="<%=request.getContextPath()%>/front-end/main.jsp">回首頁</a>
	</h2>
	<div class="container">
				...
				<form method="post" action="<%=request.getContextPath()%>/ForumArticleServlet" name="form1" enctype="multipart/form-data">
					...
					<div class="atitle">
						<input type="text" class="ititle" name="title"
							value="${param.title}" placeholder="請輸入文章標題">
					</div>
					<div class="acontent">

						<span id="tcontent">文章內容</span>
						<textarea name="content" class="editor">${param.content}</textarea>
					</div>
					<div id='container'>
					<span>請上傳圖片檔案：</span>
				    <input type="file" id="upfile" name='files' multiple>
				    <input class="acess" type="reset" id="upload" value="重設">
					<div id="picPreview" style="display: flex; width: 50%; height: 50%; flex-wrap: wrap; position: relative;"></div>	
				 </div>
					<input type="hidden" name="articleNo" value="${param.articleNo}">
					<input type="hidden" name="memNo" value="${param.memNo}"> <input
						type="hidden" name="action" value="update"> <input
						type="hidden" name="articleState"
						value="${forumArticleVO.articleState}">
	...
	<script
		src="<%=request.getContextPath()%>/ckeditor5/build/ckeditor.js"></script>
	<script
